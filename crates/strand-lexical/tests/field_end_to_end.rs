...
fn builds_writes_commits_and_queries_a_real_field_end_to_end() {
    ...
    let mut dog_rows: Vec<(u32, u32)> = all
        ...
        .collect();
    dog_rows.sort();
    assert_eq!(dog_rows, vec![(0, 1), (2, 1)], "\"dog\" occurs once each in docs 0 and 2");
    ...
    let cat_rows: Vec<(u32, u32)> = all
        ...
        .map(|(_, d, tf)| (*d, *tf))
        .collect();
    assert_eq!(cat_rows, vec![(1, 1), (2, 1)], "\"cat\" occurs once each in docs 1 and 2");
    ...
    for (term, doc_ordinal, term_freq) in &all {
        let matches = reader.lookup(term).expect("all_postings never invents a term");
