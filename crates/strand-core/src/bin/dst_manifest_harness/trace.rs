    ...
    while i < chars.len() {
        ...
        match c {
            ...
            c if c.is_ascii_digit() || (c == '-' && chars.get(i + 1).is_some_and(|d| d.is_ascii_digit())) =>
            {
                ...
                toks.push(Tok::Num(text.parse().map_err(|e| {
                    ParseError(format!("bad number {text:?}: {e}"))
                })?));
            }
            ...
            other => {
                return Err(ParseError(format!("unexpected character {other:?} in value")));
            }
        }
    }
    Ok(toks)
...
impl Parser {
    ...
    fn parse_value(&mut self) -> Result<Value, ParseError> {
        match self.peek() {
            ...
            other => Err(ParseError(format!("unexpected token starting a value: {other:?}"))),
        }
    }
...
fn value_to_optional_snapshot(v: &Value) -> Result<Option<SnapshotRec>, ParseError> {
    match v {
        ...
        other => Err(ParseError(format!("unexpected proposed/result shape: {other:?}"))),
    }
}
...
fn value_to_wlocal(v: &Value) -> Result<WLocal, ParseError> {
    Ok(WLocal {
        base_version: v
            .field("baseVersion")
            .and_then(Value::as_num)
            .ok_or_else(|| ParseError("wLocal missing baseVersion".into()))? as u64,
        next_row_id: v
            .field("nextRowId")
            .and_then(Value::as_num)
            .ok_or_else(|| ParseError("wLocal missing nextRowId".into()))? as u64,
...
fn value_to_rlocal(v: &Value) -> Result<RLocal, ParseError> {
    ...
    let result = match result_val {
        ...
        other => return Err(ParseError(format!("unexpected rLocal.result shape: {other:?}"))),
    };
    Ok(RLocal {
        retries: v
            .field("retries")
        ...
        ptr_version: v
            .field("ptrVersion")
            .and_then(Value::as_num)
            .ok_or_else(|| ParseError("rLocal missing ptrVersion".into()))? as u64,
        result,
    })
}
...
fn build_state(assignments: &[(String, String)]) -> Result<ModelState, ParseError> {
    ...
    for (name, text) in assignments {
        ...
        match name.as_str() {
            ...
            other => {
                return Err(ParseError(format!("unknown top-level trace variable {other:?}")));
            }
        }
    }
    Ok(state)
