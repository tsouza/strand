...
use strand_core::manifest::{
    CommitError, ReadError, READER_REFRESH_RETRY_LIMIT, SegmentRef, SnapshotMetadata, commit,
...
fn build_writer_trajectories(states: &[ModelState], events: &[ActionEvent]) -> Vec<WriterTrajectory> {
            ...
            match (ev.from_pc.as_str(), ev.to_pc.as_str()) {
                ...
                ("Propose", "Advance") => {
                        ...
                        && let Some(proposed) = &wl.proposed
                    {
                        ...
                        row_id_count.get_or_insert(proposed.next_row_id.saturating_sub(wl.next_row_id));
                    }
                }
                ("Propose", "Failed") => {
...
fn build_reader_trajectories(states: &[ModelState], events: &[ActionEvent]) -> Vec<ReaderTrajectory> {
    ...
    fn get(&self, key: &str) -> Result<Option<(Vec<u8>, ETag)>, StoreError> {
        if key == POINTER_KEY {
            ...
            if matches!(script.front(), Some(WriterScriptEntry::ReadFail)) {
                script.pop_front();
                return Err(StoreError::Io("scripted: ReadCurrent DefiniteFailure".into()));
            }
        }
        self.inner.get(key)
    }
    ...
    fn put_if_absent(&self, key: &str, bytes: &[u8]) -> Result<ETag, StoreError> {
        ...
        if key.starts_with(SNAPSHOT_PREFIX) {
            ...
            if matches!(script.front(), Some(WriterScriptEntry::ProposeFail)) {
                script.pop_front();
                return Err(StoreError::Io("scripted: ProposeSnapshot DefiniteFailure".into()));
            }
        }
...
impl ScriptedWriterStore<'_> {
    fn pointer_write(&self, delegate: impl FnOnce() -> Result<ETag, StoreError>) -> Result<ETag, StoreError> {
        ...
        match front {
            ...
            Some(WriterScriptEntry::AdvanceFail) => {
                self.script.borrow_mut().pop_front();
                Err(StoreError::Io("scripted: TryAdvancePointer DefiniteFailure".into()))
            }
            Some(WriterScriptEntry::AdvanceAmbiguous { landed, resolve_fails }) => {
                self.script.borrow_mut().pop_front();
                ...
                if landed {
                    match delegate() {
                        ...
                        Err(other) => {
                            self.drift_notes
                                .borrow_mut()
                                .push(format!("AmbiguousLanded scripted, real write errored: {other:?}"));
                        }
                    }
                }
                Err(StoreError::Ambiguous("scripted: TryAdvancePointer Ambiguous".into()))
            }
...
fn seed_commit(store: &InMemoryStore, snapshot: &SnapshotMetadata) {
    let path = format!("_strand/snapshots/{:020}-seed{}.json", snapshot.version, snapshot.version);
    ...
    match store.get(POINTER_KEY).expect("seed pointer read never fails") {
        Some((_, etag)) => {
            store
...
pub fn replay_writer(shared: &InMemoryStore, traj: &WriterTrajectory, is_delete_writer: bool) -> Verdict {
    ...
    let result = if is_delete_writer {
        ...
        commit_deletion_vector(&wrapper, &target, |seg| {
            ...
            bitmap.insert(0);
            let bytes =
                strand_core::deletion::build_deletion_vector(&bitmap, seg.row_id_count).expect("valid row_id_count");
            let checksum = strand_core::deletion::checksum(&bytes);
            static DV_NONCE: AtomicU64 = AtomicU64::new(0);
            let dv_path = format!("deletions/dst-{}.bin", DV_NONCE.fetch_add(1, Ordering::Relaxed));
            wrapper
...
impl ConditionalStore for ScriptedReaderStore<'_> {
    fn get(&self, key: &str) -> Result<Option<(Vec<u8>, ETag)>, StoreError> {
        if key == POINTER_KEY {
            let mut script = self.script.borrow_mut();
            if matches!(script.front(), Some(ReaderScriptEntry::PointerDefiniteFailure)) {
                script.pop_front();
                return Err(StoreError::Io("scripted: ReadPointer DefiniteFailure".into()));
            }
            return self.inner.get(key);
        }
        let mut script = self.script.borrow_mut();
        match script.front() {
            ...
            Some(ReaderScriptEntry::SnapshotDefiniteFailure) => {
                script.pop_front();
                return Err(StoreError::Io("scripted: ReadSnapshotObject DefiniteFailure".into()));
            }
            _ => {}
        }
        self.inner.get(key)
...
pub fn replay_reader(traj: &ReaderTrajectory, history: &[SnapshotRec]) -> Verdict {
    ...
    // stand-in value.
    let script: VecDeque<ReaderScriptEntry> = if matches!(traj.terminal, ReaderTerminal::RetriesExhausted) {
    ...
    match (&result, &traj.terminal) {
        ...
        (Ok(Some(real_snap)), ReaderTerminal::Found(model)) => match snapshot_matches(real_snap, model) {
            Ok(()) => Verdict::Matched,
            Err(msg) => Verdict::Drift(format!(
...
pub fn replay_trace(states: &[ModelState], delete_writer: &str) -> Result<TraceReplayResult, String> {
    ...
    for traj in &writer_trajs {
        ...
        writers.push((traj.writer.clone(), writer_has_injected_fault(&traj.script), verdict));
    }
    ...
    let history = &states.last().map(|s| s.snapshots.clone()).unwrap_or_default();
