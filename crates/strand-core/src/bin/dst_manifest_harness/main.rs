...
fn parse_args() -> Args {
    ...
    while let Some(flag) = it.next() {
        ...
        match flag.as_str() {
            ...
            "--num-per-worker" => num_per_worker = val().parse().expect("--num-per-worker is an integer"),
...
fn extract_delete_writer(cfg_text: &str) -> String {
    ...
    panic!("manifest.cfg has no `DeleteWriter = ...` line — cannot tell which writer is revise-shaped");
}
...
fn main() {
    ...
    println!("Seed: {} | workers: {} | traces requested per worker: {} | depth: {}", args.seed, args.workers, args.num_per_worker, args.depth);
    println!("Trace files found: {}", trace_files.len());
    println!("Trace files parsed successfully: {}", trace_files.len() - parse_failures.len());
    ...
    if !replay_failures.is_empty() {
        println!();
        println!("Replay-harness failures ({}) — a bug in THIS harness, not necessarily the protocol:", replay_failures.len());
    ...
    println!();
    println!("No drift: every replayed writer and reader trajectory's real outcome matched what the TLC-generated trace predicted.");
}
