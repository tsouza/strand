...
fn hybrid_rrf_fuses_a_real_lexical_ranking_and_a_real_vector_ranking_over_one_row_id_space() {
    ...
    let raw_vectors: Vec<Vec<f32>> = ROW_SCALARS
        .iter()
        .map(|&v| vec![v; padded_dims])
        .collect();
    ...
    let snapshot = commit(&store, |row_id_base| {
        ...
        let rotated_centroid_for_nav =
            rotate_fht_kac(&raw_centroid, padded_dims, rotation_payload);
        ...
        vec![write_segment(&store, "segments/hybrid-0.bin", &builder, row_id_base)]
    })
    ...
    let selected_clusters = select_nprobe_clusters(&navigation_reader, &rotated_query, 1, MetricType::L2);
    ...
    assert_eq!(candidates.len(), DOC_COUNT, "nprobe=1 with one cluster scans every row");

    let reranked = rerank(candidates, &flat_reader, row_id_base, &raw_query, MetricType::L2);
    ...
    // their order.
    let expected_distances = [(3, 0.64f32), (2, 2.56), (0, 5.76), (4, 10.24), (1, 16.0), (5, 23.04)];
    ...
    for w in expected_scores.windows(2) {
        assert!(w[0].1 > w[1].1, "expected_scores must be strictly descending: {expected_scores:?}");
    }

    assert_eq!(
        fused.iter().map(|f| f.row_id).collect::<Vec<_>>(),
        expected_scores.iter().map(|&(id, _)| id).collect::<Vec<_>>(),
    ...
    assert_eq!(vector_ranked_row_ids[0], row(3), "sanity: row 3 really is the best vector-only match");
}
